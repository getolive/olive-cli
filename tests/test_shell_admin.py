...
def test_exit_command(monkeypatch):
    with patch('olive.shell.admin.perform_graceful_exit') as pe:
        shell_admin.exit_command()
        pe.assert_called_once()

def test_help_command(monkeypatch):
    monkeypatch.setattr(shell_admin, 'get_management_commands', lambda: {':foo': lambda: None})
    with patch('olive.shell.admin.print_secondary') as ps:
...
def test_logs_command_no_pager(monkeypatch):
    monkeypatch.setattr(shell_admin, 'get_current_log_file', lambda: MagicMock(read_text=lambda: 'logdata'))
    monkeypatch.setattr(shell_admin, 'which', lambda x: False)
    with patch('olive.shell.admin.print_info') as pi, patch('olive.shell.admin.console.print') as cp, patch('olive.shell.admin.print_warning') as pw:
        shell_admin.logs_command()
...
def test_reset_state_command(monkeypatch):
    ...
    with patch('olive.shell.admin.print_warning') as pw, patch('olive.shell.admin.print_success') as ps, patch('olive.shell.admin.print_info') as pi:
...
async def test_mock_ask_command(monkeypatch):
    monkeypatch.setattr(shell_admin.llm, 'mock_ask', lambda prompt: ([], {'provider':'prov','provider_base_url':'url','model':'mod','token_count':3,'max_tokens':8,'files':[]}))
    ...
    monkeypatch.setattr(shell_admin, 'tempfile', MagicMock(NamedTemporaryFile=MagicMock(return_value=MagicMock(name='tmp', write=lambda x:None, close=lambda:None))))
...
def test_profile_command(monkeypatch):
    monkeypatch.setattr(shell_admin, 'Table', MagicMock(return_value=MagicMock(add_column=lambda *a, **k: None, add_row=lambda *a, **k: None)))
    ...
    monkeypatch.setattr(shell_admin.context, 'hydrate', lambda: None)
    monkeypatch.setattr(shell_admin, 'LLMProvider', MagicMock(return_value=MagicMock(build_payload=lambda *a, **k: None)))
    monkeypatch.setattr(shell_admin, 'console', MagicMock(print=lambda *a, **k: None))
