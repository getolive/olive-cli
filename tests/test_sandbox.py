...
if env.is_in_sandbox():  # running under !!shell pytest
    """skip this if we're running pytest inside the sandbox which wont have docker"""
    pytest.skip(
        "Docker-integration tests require a host environment.",
...
def test_is_running_status_reporting(olive_sandbox, monkeypatch):
    ...
    monkeypatch.setattr(
        "olive.sandbox._sh",
        lambda cmd, **kwargs: "running" if "inspect" in cmd else "fakeid",
    )
    monkeypatch.setattr("olive.sandbox.env.get_session_id", lambda: "testsession")
...
def test_full_cleanup_and_no_host_modification(tmp_path, monkeypatch):
    ...
    if os.path.exists(user_olive):
        assert os.stat(user_olive).st_uid == os.getuid(), (
            "Should not change user olive dir"
        )
