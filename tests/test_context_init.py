...
def test_inject_system_message(monkeypatch):
    ctx = OliveContext()
    monkeypatch.setattr(
        "olive.context.__init__.injection.append_context_injection",
        lambda content: "ok",
    )
...
def test_hydrate(monkeypatch):
    ...
    monkeypatch.setattr(
        "olive.context.__init__.injection.get_context_injections",
        lambda role="system": ["inj"],
    )
    monkeypatch.setattr(ctx, "_build_context_payload", lambda: ["cf"])
    monkeypatch.setattr(
        "olive.context.__init__.prefs",
        MagicMock(is_abstract_mode_enabled=lambda: False),
    )
...
def test_is_file_excluded(monkeypatch):
    ...
    monkeypatch.setattr(
        "olive.context.__init__.prefs",
        MagicMock(get=lambda *a, **k: [], is_abstract_mode_enabled=lambda: False),
    )
